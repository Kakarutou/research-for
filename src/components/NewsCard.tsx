...
function classify(item: NewsItem): Tab {
  ...
  const negKw = [
    ...
    "fraud", "doj investigation", "ftc probe",
...
function EarningsCard({ e, isLast }: { e: EarningsItem; isLast: boolean }) {
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
        ...
        <a
          ...
          style={{ fontSize: 11, color: "var(--gray-500)", textDecoration: "none", fontFamily: "monospace" }}
        >
          원문 →
        </a>
      </div>
      ...
      {(e.revenue || e.eps || e.netIncome || e.opProfit) && (
        <div style={{
          ...
          {e.revenue && (
            <div>
              <div style={{ fontSize: 10, color: "var(--gray-500)", marginBottom: 3, fontFamily: "monospace", letterSpacing: "0.03em" }}>REVENUE</div>
          ...
          {e.eps && (
            <div>
              <div style={{ fontSize: 10, color: "var(--gray-500)", marginBottom: 3, fontFamily: "monospace", letterSpacing: "0.03em" }}>EPS</div>
              <div style={{ fontSize: 15, fontWeight: 700, color: "#18181b", fontFamily: "monospace" }}>{e.eps}</div>
            </div>
          )}
          {(e.netIncome || e.opProfit) && (
            <div>
              <div style={{ fontSize: 10, color: "var(--gray-500)", marginBottom: 3, fontFamily: "monospace", letterSpacing: "0.03em" }}>
                {e.opProfit ? "OP PROFIT" : "NET INC"}
              </div>
              <div style={{ fontSize: 15, fontWeight: 700, color: "#18181b", fontFamily: "monospace" }}>
                {e.opProfit ?? e.netIncome}
              </div>
            </div>
          )}
        </div>
      ...
      {e.ceoQuote && (
        <div
          ...
          style={{
            ...
            fontStyle: "italic", marginBottom: e.guidance ? 8 : 0,
            cursor: e.ceoQuote.length > 130 ? "pointer" : "default",
            overflow: "hidden",
          }}
        >
          <span style={{ color: "var(--gray-400)" }}>"</span>
          {open || e.ceoQuote.length <= 130
            ? e.ceoQuote
            : e.ceoQuote.slice(0, 128) + '…'}
          <span style={{ color: "var(--gray-400)" }}>"</span>
          {e.ceoQuote.length > 130 && (
      ...
      {e.guidance && (
        <div style={{
          ...
          <span style={{ fontWeight: 700, color: "rgba(99,102,241,0.8)", marginRight: 6 }}>가이던스</span>
          {e.guidance}
        </div>
      )}
    </div>
...
export default function NewsCard({ news, earnings = [] }: { news: NewsItem[]; earnings?: EarningsItem[] }) {
  ...
  // NEW dot: earnings tab has data, or news tab has item from last 24h
            ...
            >
              ...
              {count > 0 && (
                <span style={{
                  ...
                  color: active ? "rgba(255,255,255,0.55)" : "rgba(0,0,0,0.3)",
          ...
          <>
            <div style={{ position: "absolute", inset: 0, overflowY: expanded ? "auto" : "hidden" }}>
              {newsItems.length === 0
                ...
                : newsItems.map((n, i) => (
                  <div key={i} style={{ padding: "13px 0", borderBottom: i < newsItems.length - 1 ? "1px solid var(--gray-100)" : "none" }}>
